...
function updateDeckCounts(): void {
    const playerDeckCount = document.querySelector('.cards:not(.reverse) #deck-count');
    const machineDeckCount = document.querySelector('.cards.reverse #deck-count');

    if (playerDeckCount) {
        playerDeckCount.textContent = `${villainsPlayer.length}`;
    }

    if (machineDeckCount) {
        machineDeckCount.textContent = `${villainsMachine.length}`;
    }
}
